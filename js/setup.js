...
function checkFileSize(file) {
    if (file.size > 500 * 1024) {
        alert('Image too large (' + Math.round(file.size/1024) + 'KB). Maximum is 500KB.');
        return false;
    }
    return true;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showTeamEditor = (teamId) => {
        ...
        // ── COMPACT PLAYER CARD LIST (replaces wide table) ──────
        players.forEach(p => {
            ...
            card.style.cssText = 'display:flex;align-items:center;gap:12px;padding:10px 14px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:10px;margin-bottom:6px;';
            card.innerHTML = `
                ...
                <div style="display:flex;gap:10px;font-size:0.75rem;color:#6b7280;flex-shrink:0;">
                    <span title="Goals">⚽${s.goals||0}</span>
                    <span title="Shots">🎯${s.shots||0}</span>
                    <span title="Yellow" style="color:#fbbf24;">🟨${s.yellowCards||0}</span>
                    <span title="Red" style="color:#ef4444;">🟥${s.redCards||0}</span>
                </div>
                <div style="display:flex;gap:6px;flex-shrink:0;">
                    <button class="btn-edit-player" data-pid="${p.id}" type="button"
                        style="padding:6px 12px;background:rgba(16,185,129,0.15);border:1px solid rgba(16,185,129,0.3);border-radius:6px;color:#10b981;font-size:0.8rem;font-weight:600;cursor:pointer;">✏️ Edit</button>
                    <button class="btn-delete-player" data-pid="${p.id}" type="button"
                        style="padding:6px 10px;background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.3);border-radius:6px;color:#ef4444;font-size:0.8rem;cursor:pointer;">🗑</button>
                </div>`;
        ...
        tbody.querySelectorAll('.btn-edit-player').forEach(btn => {
            btn.addEventListener('click', (e) => {
                openEditPlayerModal(e.currentTarget.dataset.pid, teamId);
            });
        });
    };
    ...
    function openEditPlayerModal(pid, teamId) {
        const player = DB.getPlayers().find(p => p.id === pid);
        if (!player) return;
        editPlayerTeamId = teamId; editPlayerPhotoFile = null;
        ...
        preview.innerHTML = '<img src="' + DB.getPlayerAvatar(player, team) + '" style="width:100%;height:100%;object-fit:cover;">';
    ...
    document.getElementById('ep-photo-input').addEventListener('change', function() {
        ...
        compressImage(file, 300, 0.8, function(compressed) {
            document.getElementById('ep-photo-preview').innerHTML = '<img src="' + compressed + '" style="width:100%;height:100%;object-fit:cover;">';
        });
    });
    ...
    document.getElementById('ep-save-btn').addEventListener('click', async () => {
        ...
        if (editPlayerPhotoFile) {
            const player = DB.getPlayers().find(p => p.id === pid);
            const cloudUrl = await uploadPhoto(editPlayerPhotoFile, 'player', pid, player?.photo);
            photoData = cloudUrl || await new Promise(r => compressImage(editPlayerPhotoFile, 300, 0.8, r));
        }
        const updates = {
